function dx = hippocampus_parameterized(t, x, param)
%% Parametervector
...
R_nb = [ cos(psi)*cos(theta) -sin(psi)*cos(phi)+cos(psi)*sin(theta)*sin(phi)  sin(psi)*sin(phi)+cos(psi)*cos(phi)*sin(theta);...
         sin(psi)*cos(theta)  cos(psi)*cos(phi)+sin(phi)*sin(theta)*sin(psi) -cos(psi)*sin(phi)+sin(phi)*sin(psi)*cos(phi);...
...
dx(1:6) = J_Theta*nu;
dx(7:12) = (M_RB + M_A)\(tau - D*nu - C_RB*nu - C_A*nu + g + g_0);
